...
echo 'setenv MAINDIR "${9}"' >> scripts/run_${TAG}.csh
echo 'setenv MODELNAME "${1}_${2}${3}_${4}_${5}"' >> scripts/run_${TAG}.csh
...
echo 'cp -a store_${5}/slha/${1}_${2}${3}.slha ${MADGRAPHDIR}/models/${MODELNAME}/param_card.dat' >> scripts/run_${TAG}.csh
...
echo 'setenv EVENT "${7}"' >> scripts/run_${TAG}.csh
echo 'setenv SEED `cat ${MAINDIR}/test/randomSeedPi.txt | awk -v nline=${4} '"'"'{if (NR == nline) print $1}'"'"'`' >> scripts/run_${TAG}.csh
echo 'setenv XQCUT "${8}"' >> scripts/run_${TAG}.csh
...
# Switch to your working directory below
echo -e 'cd ../BRIDGE\n' >> scripts/run_${TAG}.csh
echo -e './runBRIsusy.exe ../models/${MODELNAME}/particles.dat ../models/${MODELNAME}/param_card.dat ../models/${MODELNAME}/interactions.dat ../models/${MODELNAME}/ ../models/${MODELNAME}/${MODELNAME} blist n3 x1+ x1- n2 elist ${SEED} 50000 5 Y\n' >> scripts/run_${TAG}.csh
echo 'if ( ${5} == "HH" ) then' >> scripts/run_${TAG}.csh
...
echo 'else if ( ${5} == "HZ" ) then' >> scripts/run_${TAG}.csh
echo 'cat ../models/${MODELNAME}/n2_decays.table | awk '\''{if (NF == 3 && $1 == "n1" && $2 == "z") {printf "%s %s %0.6f\n", $1,$2, 0.500000} else if (NF == 3 && $1 == "n1" && $2 == "h1" ) {printf "%s %s %0.6f\n", $1,$2, 0.500000} else if (NF == 3 && $1 != "#") {printf "%s %s %0.6f\n", $1,$2, 0.000000} else if (NF == 4 && $1 != "#") {printf "%s %s %s %0.6f\n", $1,$2, $3, 0.000000} else if ($1 == "#") {print}}'\'' > ../models/${MODELNAME}/n2_decays.table_temp' >> scripts/run_${TAG}.csh
echo 'else if ( ${5} == "ZZ" ) then' >> scripts/run_${TAG}.csh
echo 'cat ../models/${MODELNAME}/n2_decays.table | awk '\''{if (NF == 3 && $1 == "n1" && $2 == "z") {printf "%s %s %0.6f\n", $1,$2, 1.000000} else if (NF == 3 && $1 == "n1" && $2 == "h1" ) {printf "%s %s %0.6f\n", $1,$2, 0.000000} else if (NF == 3 && $1 != "#") {printf "%s %s %0.6f\n", $1,$2, 0.000000} else if (NF == 4 && $1 != "#") {printf "%s %s %s %0.6f\n", $1,$2, $3, 0.000000} else if ($1 == "#") {print}}'\'' > ../models/${MODELNAME}/n2_decays.table_temp' >> scripts/run_${TAG}.csh
echo -e 'endif\n' >> scripts/run_${TAG}.csh
...
echo -e 'cd ${MAINDIR}\n' >> scripts/run_${TAG}.csh
echo -e './ExplicitHiggsDecayer.sh ${6} ${MODELNAME} ${MADGRAPHDIR} ${MAINDIR}\n' >> scripts/run_${TAG}.csh
echo 'cd ${MADGRAPHDIR}/${MODELNAME}/Events/${MODELNAME}' >> scripts/run_${TAG}.csh
echo '${MAINDIR}/test/lheDataOps.py unweighted_events.lhe ${MODELNAME}_${6}.lhe' >> scripts/run_${TAG}.csh
echo -e 'cp ${MODELNAME}_${6}_official.lhe ${MAINDIR}/store_${5}/lhe/.\n' >> scripts/run_${TAG}.csh
...
do
  ...
  j=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $1}'` # chargino mass
  k=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $2}'` # random seed
  l=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $3}'` # higgsino decay mode
  m=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $4}'` # higgs boson decay mode
  n=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $5}'` # number of events
  o=`cat $FILENAME | awk -v number=$i '{if (number == NR) print $6}'` # xqcut value

  NAME="${TAG}_${NAMEONE}${j}_${k}_${l}_${m}"
  ARGUMENT="${TAG} ${NAMEONE} ${j} ${k} ${l} ${m} ${n} ${o} ${MAINDIR}"
